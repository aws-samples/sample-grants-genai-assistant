"""
EU Grant Search V2 Initiator Lambda - AgentCore Native

This Lambda function initiates async EU grant search using AgentCore Runtime's
...
_cached_eu_cache_bucket = None

def get_eu_cache_bucket():
    """Get EU cache bucket name from environment variable"""
    global _cached_eu_cache_bucket
    
    if _cached_eu_cache_bucket:
        return _cached_eu_cache_bucket
    
    eu_cache_bucket = os.environ.get('EU_CACHE_BUCKET')
    if not eu_cache_bucket:
        logger.error("[EU V2] EU_CACHE_BUCKET environment variable not set")
        raise ValueError("EU_CACHE_BUCKET environment variable is required")
    
    _cached_eu_cache_bucket = eu_cache_bucket
    logger.info(f"[EU V2] Using EU cache bucket: {eu_cache_bucket}")
    return eu_cache_bucket
...
def get_agent_arn_from_export():
    ...
    try:
        logger.info(f"[EU V2] Getting agent ARN from CloudFormation export: {AGENT_ARN_EXPORT_NAME}")
        ...
        for export in response.get('Exports', []):
            if export['Name'] == AGENT_ARN_EXPORT_NAME:
                ...
                logger.info(f"[EU V2] Found agent ARN: {_cached_agent_arn}")
                return _cached_agent_arn
        
        logger.error(f"[EU V2] CloudFormation export not found: {AGENT_ARN_EXPORT_NAME}")
        raise ValueError(f"CloudFormation export not found: {AGENT_ARN_EXPORT_NAME}")
        
    except Exception as e:
        logger.error(f"[EU V2] Failed to get agent ARN from CloudFormation export: {e}")
        raise
...
def discover_appsync_endpoint():
    ...
    try:
        logger.info(f"[EU V2] Discovering AppSync endpoint for API ID: {GRAPHQL_API_ID}")
        ...
        logger.info(f"[EU V2] Discovered AppSync endpoint: {endpoint}")
        return endpoint
    except Exception as e:
        logger.error(f"[EU V2] Failed to discover AppSync endpoint: {e}")
        raise


# Agent configuration - read from CloudFormation export (set by CDK)
EU_GRANTS_V2_AGENT_ARN = None  # Will be loaded on first invocation

def get_eu_grants_v2_agent_arn():
    """Get agent ARN (cached after first call)"""
    global EU_GRANTS_V2_AGENT_ARN
    if not EU_GRANTS_V2_AGENT_ARN:
        EU_GRANTS_V2_AGENT_ARN = get_agent_arn_from_export()
    return EU_GRANTS_V2_AGENT_ARN


def lambda_handler(event, context):
    """
    Main Lambda handler for initiating async EU grant search (V2)
    
    Expected event structure from GraphQL:
    {
        "info": {"fieldName": "startEuGrantSearchV2"},
        "arguments": {
            "input": {
                "sessionId": "eu_search_12345",
    ...
    """
    try:
        logger.info(f"[EU V2] Grant search initiator invoked with event: {json.dumps(event)}")
        ...
        if cognito_user_id:
            logger.info(f"[EU V2] Request from Cognito user: {cognito_user_id}")
        else:
            logger.warning("[EU V2] Request without user authentication")
        ...
        logger.info(f"[EU V2] Processing startEuGrantSearchV2 mutation")
        
        return handle_start_eu_grant_search_v2(arguments, cognito_user_id)
            
    except Exception as e:
        logger.error(f"[EU V2] Error in grant search initiator: {str(e)}")
        ...
        error_result = {
            ...
            'error': str(e),
            'message': f'EU search failed: {str(e)}',
            'version': 'V2'
        }
        ...
        try:
            ...
            logger.error("[EU V2] Failed to write error event to DynamoDB")
        
        return error_result

def handle_start_eu_grant_search_v2(arguments: Dict[str, Any], cognito_user_id: str = None) -> Dict[str, Any]:
    """
    Start an EU grant search using AgentCore Runtime native async pattern
    ...
    try:
        ...
        sources = search_input.get('sources', ['EU_FUNDING'])
        
        logger.info(f"[EU V2] Starting EU grant search for session {session_id} with query: {query}")
        ...
        if GUARDRAIL_ID and GUARDRAIL_VERSION:
            ...
            if guardrail_response.get('action') == 'GUARDRAIL_INTERVENED':
                logger.warning(f"[EU-V2] Guardrail BLOCKED search query: {query[:100]}")
                return {
                    "eventType": "SEARCH_ERROR",
                    "sessionId": session_id,
                    "timestamp": datetime.utcnow().isoformat() + "Z",
        ...
        logger.info(f"[EU V2] Created search status record for session {session_id}")
        ...
        # Get EU grants cache bucket name from environment variable
        eu_cache_bucket = get_eu_cache_bucket()
        ...
        agent_payload = {
            ...
            "version": "V2",
            "tableNames": {
                "euGrantRecords": os.environ.get('EU_GRANT_RECORDS_TABLE'),
                "searchEvent": os.environ.get('SEARCH_STATUS_TABLE'),
                "userProfile": os.environ.get('USER_PROFILE_TABLE')
            },
            "euCacheBucket": eu_cache_bucket,
            "appsyncEndpoint": appsync_endpoint,
            "graphqlApiId": GRAPHQL_API_ID,
        }
        
        logger.info(f"[EU V2] Calling AgentCore Runtime for session {session_id}")
        agent_arn = get_eu_grants_v2_agent_arn()
        logger.info(f"[EU V2] Agent ARN: {agent_arn}")
        logger.info(f"[EU V2] Payload: {json.dumps(agent_payload, cls=DecimalEncoder)}")
        ...
        if len(session_id) < 33:
            ...
            logger.info(f"[EU V2] Padded session ID: {session_id} -> {agentcore_session_id}")
        ...
        logger.info("[EU V2] AgentCore invocation successful")
        
        # Process AgentCore response
        response_body = response['response'].read()
        agent_response = json.loads(response_body)
        
        logger.info(f"[EU V2] Agent response: {agent_response}")
        ...
        logger.error(f"[EU V2] Error starting EU grant search: {str(e)}")
...
def set_default_filters(filters: Dict[str, Any]) -> Dict[str, Any]:
    """Set default values for EU search filters"""
    defaults = {
        "minAmount": 0,
        "maxAmount": None,
        "programmes": [],
...
def create_search_status_record(
    ...
    """Create initial EU search status record"""
    now = datetime.utcnow()
    ...
    return {
        ...
        "data": {
            ...
            "progress": {
                ...
                "message": "EU search started - agent processing"
            },
            "grantsFound": 0,
            "version": "V2",
            "agentArn": get_eu_grants_v2_agent_arn()
        },
...
def write_search_event(session_id: str, event_data: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"[EU V2] Published search event for session {session_id}: {event_data.get('eventType')}")
        
    except Exception as e:
        logger.error(f"[EU V2] Error publishing search event: {str(e)}")
